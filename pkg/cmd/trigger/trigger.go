...
func (o *TriggerOptions) TriggerPipeline(jenkinsClient gojenkins.JenkinsClient, gitInfo *gits.GitRepository) error {
	...
	if err != nil {
		return errors.Wrapf(err, "cannot tail build for %s", job.FullName)
	}
	return err
}
...
func (o *TriggerOptions) getOrCreateMultiBranchPipeline(jenkinsClient gojenkins.JenkinsClient, gitInfo *gits.GitRepository) (result *gojenkins.Job, err error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "failed to scan multibranch project %s", job.FullName)
	}
	...
	err = gojenkins.Poll(1*time.Second, 60*time.Second, fmt.Sprintf("poll for build of %s", job.FullName), pollBuildCB)
	if err != nil {
		return &job, err
	}
	log.Logger().Infof("cancelling build %s/%d", job.FullName, buildnumber)
	err = jenkinsClient.StopBuild(job, buildnumber)

	return &job, err
}

func (o *TriggerOptions) triggerAndWaitForBuildToStart(jenkins gojenkins.JenkinsClient, job gojenkins.Job, buildStartWaitTime time.Duration) (result *gojenkins.Build, err error) {
	previousBuildNumber := 0
	previousBuild, err := jenkins.GetLastBuild(job)
	if err != nil {
		if !is404(err) {
			return nil, fmt.Errorf("error finding last build for %s due to %v", job.FullName, err)
		}
	} else {
	...
	if err != nil {
		if !is404(err) {
			return nil, fmt.Errorf("error triggering build %s due to %v", job.FullName, err)
		}
	}
	...
	fn := func() (bool, error) {
		...
		if err != nil {
			if !is404(err) {
				return false, fmt.Errorf("error finding last build for %s due to %v", job.FullName, err)
			}
		} else {
			buildNumber = build.Number
		}
